...
export function createBolusUI(cardId, medKey, config = {}) {
  ...
  return `
    <div class="medication-header mb-3">
      <h4 class="medication-title">${medicationsDB[medKey]?.name || 'Selecione a Medicação'}</h4>
    </div>
...
export function createInfusionUI(cardId, medKey, config = {}) {
  ...
  return `
    ...
    ${med.doseOptions && !config.isLocked ? `
      <div class="mb-3">
        <label class="form-label infusion-dose-select-text">Perfil de Dose</label>
